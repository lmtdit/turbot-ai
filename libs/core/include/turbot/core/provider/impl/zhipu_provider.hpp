...
/// Zhipu AI (智谱) GLM provider implementation
/// Supports GLM series models
/// API docs: https://open.bigmodel.cn/dev/api
class TURBOT_CORE_API ZhipuProvider : public Provider {
public:
    /// Default API base URL
    static constexpr std::string_view DEFAULT_BASE_URL = "https://open.bigmodel.cn/api/paas/v4";

    /// Create Zhipu provider with configuration
    explicit ZhipuProvider(const ProviderConfig& config);

    /// Create Zhipu provider with API key
    explicit ZhipuProvider(const std::string& api_key);

    ~ZhipuProvider() override = default;

    // ===== Provider interface =====

    [[nodiscard]] std::string id() const override { return "zhipu"; }
    [[nodiscard]] std::string name() const override { return "智谱AI"; }
